...
function scoreMove(pos: Position, target: Position, away: boolean): { pos: Position; score: number } {
    ...
    // For 'away' moves, we want to maximize distance, so we minimize the negative distance.
...
/**
 * Chooses a move for a unit from its top 3 best options, with optional cover-seeking behavior.
...
function chooseMove(u: Unit, target: Position, all: Unit[], away: boolean, preferCover: boolean): Position {
    ...
    if (preferCover) {
        const coverMoves = possibleMoves.filter(p => isPositionInCoverFrom(p, target, all));
        if (coverMoves.length > 0) {
            const scoredMoves = coverMoves.map(p => scoreMove(p, target, away));
    ...
    const scoredMoves = possibleMoves.map(p => scoreMove(p, target, away));
    scoredMoves.sort((a, b) => a.score - b.score);
    const topCandidates = scoredMoves.slice(0, 3);
