...
function findBestGrenadeSpot(u: Unit, enemies: Unit[], allUnits: Unit[]): { pos: Position, score: number } | null {
    ...
    potentialSpots.forEach(spotKey => {
        ...
        // Prevent targeting the grenadier's own current cell.
        if (x === u.x && y === u.y) {
            return;
        }
...
export function grenadierDecide(u: Unit, all: Unit[]) {
    ...
    if (u.grenCooldown === 0 && desperateCondition) {
        ...
        let blastCenter: Position | null = null;
        if (adjacentEnemies.length > 0) {
            // Target the adjacent enemy that is weakest to secure a kill.
            adjacentEnemies.sort((a,b) => a.hp - b.hp);
            blastCenter = { x: adjacentEnemies[0].x, y: adjacentEnemies[0].y };
        } else if (isCornered || isTeamEndangered) {
            // No adjacent enemies, but still desperate. Find the best possible throw nearby.
            const bestThrow = findBestGrenadeSpot(u, enemies, all);
            if(bestThrow) {
                blastCenter = bestThrow.pos;
            }
        }

        // Only proceed if a valid blast center was found.
        if (blastCenter) {
            ...
            if (enemyValueHit > friendlyValueHit) {
                const distanceToBlast = Math.max(Math.abs(u.x - blastCenter.x), Math.abs(u.y - blastCenter.y));
                if (distanceToBlast <= C.GRENADE_RANGE) {
