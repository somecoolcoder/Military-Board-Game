...
function calculateTeamStrength(teamUnits: Unit[]): number {
    return teamUnits.reduce((total, u) => {
        ...
        const avgDamage = C.UNIT_AVG_DAMAGE[u.type] || 0;
        // Only count revealed spies in strength calculation for their real team
        const effectiveHp = (u.type === 'spy' && !u.spyRevealed) ? 0 : u.hp;
        
        const healthComponent = (effectiveHp / u.maxHP) * 100; // Normalized HP percentage
        
        // Strength is a combination of the unit's value, its current health, and its damage potential.
        return total + value * (healthComponent + avgDamage);
    }, 0);
}
...
function renderAnimations() {
    ...
    for (const anim of state.effects) {
        ...
        switch (anim.type) {
            ...
            case 'grenade_projectile':
                if (!anim.from || !anim.to) continue;
                el.className = 'grenade-projectile';
                el.innerText = '💣';
                const fromGrenPos = getPixelPos(anim.from);
                const toGrenPos = getPixelPos(anim.to);

                const startGrenX = fromGrenPos.left + size / 2;
                const startGrenY = fromGrenPos.top + size / 2;
                const endGrenX = toGrenPos.left + size / 2;
                const endGrenY = toGrenPos.top + size / 2;
                
                // Centering the emoji. font size is 24px, so offset is 12px.
                el.style.left = `${startGrenX - 12}px`;
                el.style.top = `${startGrenY - 12}px`;
                el.style.transform = 'scale(1)';
                
                animationLayer.appendChild(el);

                requestAnimationFrame(() => {
                    el.style.left = `${endGrenX - 12}px`;
                    el.style.top = `${endGrenY - 12}px`;
...
async function askAIAdvisor() {
    ...
    try {
        ...
        const settingsData = {
            ...
            'Unit Stats': {
                'Soldier HP': C.HP_SOLDIER, 'Soldier Damage': `${C.DAMAGE_MIN}-${C.DAMAGE_MAX}`, 'Soldier Bandages': C.SOLDIER_BANDAGES,
                ...
                'Commando HP': C.HP_COMMANDO, 'Commando Damage': `${C.COMMANDO_MIN}-${C.COMMANDO_MAX}`, 'Commando Max Targets': C.COMMANDO_MAX_TARGETS, 'Commando Cooldowns (Atk/Move)': `${C.COMMANDO_ATTACK_CD}/${C.COMMANDO_MOVE_CD}`, 'Commando Bandages': C.COMMANDO_BANDAGES,
